...
3. **職責分離**：`umbra-orchestrator` 嚴禁直接存取或修改 `src/` 中的任何原始碼。所有代碼變更必須透過：修改藍圖 ➡️ `@umbra-reviewer` 審核（除非帶有 `--direct`） ➡️ `@umbra-coder` 投影實作。
...
- **`--worktree` 隔離處理**：
  ...
  - **若包含 `--worktree`**：在開始本次任務的任何藍圖或程式碼修改之前，請先載入 `using-git-worktrees` 技能或執行 `git worktree add` 建立一個全新的獨立 Worktree，並切換至該 Worktree 執行後續所有動作，**嚴禁影響主分支 (`master`/`main`)**。
- **`--ask` 訪談對齊處理**：
...
- **檢查 `--direct` 旗標**：
  ...
  - **若未包含 `--direct`**：執行標準審查流程，發派給 Coder 執行前，必須先調用 `@umbra-reviewer` 進行獨立審核與修復：
    - 使用 `task` 工具呼叫 `@umbra-reviewer`，將使用者目標/原始需求、已修改藍圖清單與內容、高嚴謹模式標記等傳遞給他。
